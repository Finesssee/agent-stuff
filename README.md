...
* [`orchestrator-controller.ts`](pi-extensions/orchestrator-controller.ts) - Explicit `orchestrate` tool and `/orchestrate` command family with durable run artifacts, `/orchestrate status`, `/orchestrate inspect`, a dedicated machine-readable planner leg, mission escalation through Mission Control, and planner -> worker fanout -> reviewer orchestration on top of the subagent bridge. Worker fanout now uses a heterogeneous pool with `composer-2-fast` as the primary worker and additional smart-routed worker models available for parallel slices.
* [`orchestrator-runtime.ts`](pi-extensions/orchestrator-runtime.ts) - Durable orchestrator runtime state under `~/.pi/agent/orchestrator/`, including active run state, per-run JSON/Markdown artifacts, widget/status helpers, and the actual worker-model mix used in a run.
